...
OUTPUT_STREAM* make_jpg_image(RAW_YCbCr_IMAGE* raw_ycbcr_image,
	...
	xil_printf("Byte buffer alociran\r\n");
	...
	for (x = 0; x < n_x_blocks; x++) {
		for (y = 0; y < n_y_blocks; y++) {
			...
			for (int u = 0; u < 8; u++) {
				for (int v = 0; v < 8; v++) {
					temp_buffer[temp_buffer_index] =
							(signed char) ycbcr_block.ycbcr[u][v].Y;
					++temp_buffer_index;
				}
			}
			for (int n = 0; n < 64; n++) {
			...
			for (int u = 0; u < 8; u++) {
				for (int v = 0; v < 8; v++) {
					temp_buffer[temp_buffer_index] =
							(signed char) ycbcr_block.ycbcr[u][v].Cb;
					++temp_buffer_index;
				}
			}
			for (int n = 0; n < 64; n++) {
			...
			for (int u = 0; u < 8; u++) {
				for (int v = 0; v < 8; v++) {
					temp_buffer[temp_buffer_index] =
							(signed char) ycbcr_block.ycbcr[u][v].Cr;
					++temp_buffer_index;
				}
			}
			for (int n = 0; n < 64; n++) {
	...
	xil_printf("Cik cak ispis napravljen\r\n");
	...
	while (block_counter < number_of_blocks) {
		...
		y_buffer_index = code_block(y_buffer, y_buffer_index, &binary_buffer,
				dc_luminance, ac_luminance, ac_zrl_lum, ac_eob_lum);
		// code one block of cb component
		cb_buffer_index = code_block(cb_buffer, cb_buffer_index, &binary_buffer,
				dc_chrominance, ac_chrominance, ac_zrl_chrom, ac_eob_chrom);
		// code one block of cr component
		cr_buffer_index = code_block(cr_buffer, cr_buffer_index, &binary_buffer,
				dc_chrominance, ac_chrominance, ac_zrl_chrom, ac_eob_chrom);
